...
Ricapitolando, il risultato prodotto in questo sprint è il seguente:
\begin{itemize}
...
La struttura funzionale del progetto è stata ampliata dallo scorso sprint, in quanto attualmente i componenti del sistema sono:
...
Per lo sviluppo del backend abbiamo deciso di utilizzare Rust, un linguaggio di programmazione che offre prestazioni elevate e una gestione della memoria molto efficiente.\\
Il linguaggio ci ha permesso di sviluppare i due web server in modo molto efficiente e veloce, inoltre la sua gestione della concorrenza ci ha permesso di sviluppare un sistema molto performante.\\

Lo sviluppodei due web server è stato possibile grazie all'utilizzo della libreria \texttt{actix-web}, una libreria molto performante e che offre un'interfaccia molto semplice per la gestione delle richieste HTTP.\\
Attraverso di essa siamo riusciti a gestire tutte le richieste provenienti dai due applicativi desktop e mobile in modo molto efficiente e veloce, riuscendo a gestire tutte le casistiche possibili.\\

Anche la test suite è stata scritta in Rust, utilizzando la libreria \texttt{actix-web} per testare le funzionalità dei due web server.

\subsubsection{Frontend}
Siccome era nostra idea sviluppare due applicativi, uno mobile e uno desktop, abbiamo deciso di utilizzare il framework Flutter per lo sviluppo del frontend.\\
Flutter è un framework sviluppato da Google studiato per lo sviluppo dell'interfaccia di applicativi mobile, ma che permette anche di sviluppare l'interfaccia di applicazioni desktop.\\
